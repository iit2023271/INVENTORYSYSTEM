...
function CustomerDetails() {
  ...
  const normalOrders = customer.normalOrders.filter((o) => o.status === "Completed");
  const customOrders = customer.customOrders.filter((o) => o.status === "Completed");
  ...
  return (
    <>
      ...
      <div className="container" style={{ maxWidth: 480, margin: "0 auto", padding: 16 }}>
        ...
        {/* Back button */}
        <button onClick={() => navigate("/customers")} style={btn("#f0f0f0", "#333")}>
          ← Back
        </button>
        ...
        <div style={{ textAlign: "center", margin: "20px 0" }}>
          ...
          <div style={{
            ...
            color: "#1976d2", fontSize: 26, fontWeight: "bold",
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {[
            { label: "Total",  value: normalOrders.length + customOrders.length, color: "#2e7d32", bg: "#e8f5e9" },
            { label: "🛒 Normal", value: normalOrders.length, color: "#1565c0", bg: "#e3f2fd" },
            { label: "📝 Custom", value: customOrders.length, color: "#e65100", bg: "#fff3e0" },
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {["normal", "custom"].map((t) => (
            ...
            >
              {t === "normal" ? `🛒 Normal (${normalOrders.length})` : `📝 Custom (${customOrders.length})`}
            </button>
          ))}
        </div>
          ...
          normalOrders.length === 0
            ...
            : normalOrders.map((order) => (
                <div key={order._id} style={card("#4CAF50")}>
                  ...
                  {/* When it was placed */}
                  <p style={meta}>🕒 Placed: {new Date(order.createdAt).toLocaleString("en-IN")}</p>

                  {/* When it was delivered (only shown if field exists) */}
                  {order.completedAt && (
                    <p style={meta}>🚚 Delivered: {new Date(order.completedAt).toLocaleString("en-IN")}</p>
                  )}

                  {/* Items list */}
                  <div style={{ background: "#f9f9f9", borderRadius: 6, padding: "10px 12px", margin: "10px 0" }}>
                    {order.items.map((item, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", fontSize: 14 }}>
                        <span>{item.name} × {item.quantity}</span>
                        <span style={{ color: "#2e7d32", fontWeight: 600 }}>₹{item.subTotal}</span>
                      </div>
                    ))}
                  </div>

                  {/* Grand total */}
                  <div style={{ textAlign: "right", fontWeight: "bold", fontSize: 16, color: "#2e7d32" }}>
          ...
          customOrders.length === 0
            ...
            : customOrders.map((order) => (
                <div key={order._id} style={card("#FF9800")}>
                  ...
                  <p style={meta}>📅 Scheduled: {new Date(order.deliveryDate).toLocaleString("en-IN")}</p>
                  ...
                  <div style={{ display: "flex", gap: 8, margin: "10px 0" }}>
                    {[
                      ...
                      { label: "Advance", value: order.advancePaid,  color: "#1565c0" },
                      { label: "Balance", value: order.balanceAmount, color: "#e65100" },
                    ].map(({ label, value, color }) => (
                      <div key={label} style={{ flex: 1, textAlign: "center", background: "#fafafa", borderRadius: 6, padding: 8 }}>
                  ...
                  {order.notes && (
                    <div style={{ background: "#fff8e1", borderRadius: 6, padding: 10, fontSize: 13, color: "#555" }}>
                      📋 {order.notes}
                    </div>
                  )}
                </div>
...
function EmptyState({ text }) {
  return <p style={{ textAlign: "center", color: "#aaa", padding: 40 }}>{text}</p>;
}
...
// Small status badge
const badge = (bg) => ({
  background: bg, color: "#fff", padding: "3px 10px",
...
const btn = (bg, color = "#fff") => ({
  ...
  background: bg, color, cursor: "pointer", fontWeight: 600, marginBottom: 8,
});
